...
namespace NodeCanvas.Editor{
	...
	public class GraphEditor : EditorWindow{
		...
		void OnEnable(){
		    ...
		    EditorApplication.playmodeStateChanged += () => { welcomeShown = true; };
			Repaint();
            //UNITY 5
		...
		void OnInspectorUpdate(){
			if (currentGraph != null && !currentGraph.isRunning)
				Repaint();
		}
		...
		void DoSmoothPan(){
			...
			if ( Mathf.Abs( (smooth - pan).magnitude) > 0 ){
				smooth = new Vector2( Mathf.Round(smooth.x), Mathf.Round(smooth.y) ); //pixel perfect correction
				pan = Vector2.SmoothDamp(pan, smooth, ref _panVelocity, 0.0002f);
				Repaint();
			} else {
				smoothPan = null;
			}
		...
		void DoSmoothZoom(){
			...
			if ( Mathf.Abs(zmooth - zoomFactor) > 0 ){
				...
				zoomFactor = Mathf.SmoothDamp(zoomFactor, zmooth, ref _zoomVelocity, 0.0002f);
		...
		void OnGUI(){
			...
			//canvas background
			GUI.Box(canvasRect, string.Format("{0}\n{1}", currentGraph.GetType().Name, "@NodeCanvas v2.1"), "canvasBG");
			...
			GUI.BeginGroup(canvasRect);
				...
				GUI.BeginGroup(totalCanvas);
					...
					//simple transformation info in the center. Not mandatory
					GUI.color = new Color(1,1,1,0.08f);
					GUI.Box(viewRect, virtualPanPosition.ToString());
					GUI.color = Color.white;
					//
					...
					BeginWindows();
					currentGraph.ShowNodesGUI(e, viewRect, firstPass, mousePosInCanvas, zoomFactor);
					EndWindows();		
		...
		void HandleEvents(Event e){
			...
			if (e.type == EventType.ScrollWheel && Graph.allowClick){
				if (canvasRect.Contains(e.mousePosition))
					ZoomAt(e.mousePosition, -e.delta.y > 0? 0.5f : -0.5f );
			}
			...
			if ( (e.button == 2 && e.type == EventType.MouseDrag && canvasRect.Contains(e.mousePosition)) /* || (e.button == 0 && e.type == EventType.MouseDrag && e.shift && e.isMouse) */ ){
				pan += e.delta;
				smoothPan = null;
				smoothZoomFactor = null;
			}
...
*/
		...
		void DoCanvasRectSelection(Rect container){
			if (Graph.allowClick && e.type == EventType.MouseDown && e.button == 0 && !e.alt && !e.shift){
...
*/
		...
		void ShowBreadCrumbNavigation(Graph root){
			...
			//if something selected the inspector panel shows on top of the breadcrub. If external inspector activated it doesnt matter, so draw anyway.
			...
			var graphInfo = string.Format("<color=#ff4d4d>({0})</color>", targetOwner != null && targetOwner.graph == root && targetOwner.graphIsLocal? "Binded" : ( EditorUtility.IsPersistent(root)? "Asset" : "Instance" ) );
