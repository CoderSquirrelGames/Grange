	...
	partial class Node{
		    ...
		    if (Graph.allowClick && e.button == 1 && e.type == EventType.MouseUp){
		    	...
		    	//Single node menu
		            ...
		            if (graph.primeNode != this && allowAsPrime)
			            menu.AddItem (new GUIContent ("Set Start (CTRL+Click)"), false, ()=> {graph.primeNode = this;});
		...
		void HandleNodePosition(Event e){
		    if (Graph.allowClick && e.button != 2){
	    		...
	    		if (e.type == EventType.MouseDrag && Graph.multiSelection.Count != 0){
		...
		//If the node implements ITaskAssignable this is shown for the user to assign a tas
		void TaskAssignableGUI(){
			...
			if (this is ITaskAssignable){
				...
				if (taskType != null){
					EditorUtils.TaskField(assignable.task, graph, taskType, (t)=> {assignable.task = t;} );
				}
			}
		}
		...
		protected void DrawDefaultInspector(){
			EditorUtils.ShowAutoEditorGUI(this, EditorUtility.IsPersistent(graph));
		}
