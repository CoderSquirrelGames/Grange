			...
			if (Graph.allowClick && e.type == EventType.MouseDown && e.button == 1 && areaRect.Contains(e.mousePosition)){
				...
				if (assignable != null){
					...
					if (Task.copiedTask != null){
						menu.AddItem(new GUIContent(string.Format("Pase Assigned Task ({0})", Task.copiedTask.name)), false, ()=>
						{
							if (assignable.task == Task.copiedTask)
								return;

							if (assignable.task != null){
								if (!EditorUtility.DisplayDialog("Paste Task", string.Format("Connection already has a Task assigned '{0}'. Replace assigned task with pasted task '{1}'?", assignable.task.name, Task.copiedTask.name), "YES", "NO"))
									return;								
							}
