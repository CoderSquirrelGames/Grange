...
namespace NodeCanvas.Framework{
	...
	partial class Graph {
		...
		public void ShowGraphControls(Event e, Vector2 canvasMousePos){
			...
			DoContextMenu(e, canvasMousePos);
		...
		void ShowToolbar(Event e){
			...
			if (GUILayout.Button("File", EditorStyles.toolbarDropDown, GUILayout.Width(50))){
				...
				menu.AddItem (new GUIContent ("Import JSON"), false, ()=>
				{
	                ...
	                var path = EditorUtility.OpenFilePanel( string.Format("Import '{0}' Graph", this.GetType().Name), "Assets", fileExtension);
				...
				{
					var path = EditorUtility.SaveFilePanelInProject (string.Format("Export '{0}' Graph", this.GetType().Name), "", fileExtension, "");
	                if (!string.IsNullOrEmpty(path)){
			...
			if (GUILayout.Button("Prefs", EditorStyles.toolbarDropDown, GUILayout.Width(50))){
				...
				menu.AddItem (new GUIContent ("Show Task Summary Info"), NCPrefs.showTaskSummary, ()=> {NCPrefs.showTaskSummary = !NCPrefs.showTaskSummary;});
				menu.AddItem (new GUIContent ("Node Help"), NCPrefs.showNodeInfo, ()=> {NCPrefs.showNodeInfo = !NCPrefs.showNodeInfo;});
				...
				menu.AddItem (new GUIContent ("Curve Mode/Smooth"), NCPrefs.curveMode == 0, ()=> {NCPrefs.curveMode = 0;});
				menu.AddItem (new GUIContent ("Curve Mode/Stepped"), NCPrefs.curveMode == 1, ()=> {NCPrefs.curveMode = 1;});
			...
			/*//DROP GRAPHOWNER JUMP SELECTION
			if (GUILayout.Button(owner != null? owner.gameObject.name : "[NONE]", EditorStyles.toolbarDropDown, GUILayout.Width(120))){
				var menu = new GenericMenu();
				foreach(var _o in FindObjectsOfType<GraphOwner>().Where(o => o.graphType == this.GetType() ) ){
					var o = _o;
					menu.AddItem (new GUIContent(o.gameObject.name), false, ()=> { Selection.activeGameObject = o.gameObject; });
				}
				menu.ShowAsContext();
			}
			*/////////////////////////////////////
		...
		void DoContextMenu(Event e, Vector2 canvasMousePos){
		...
		void ShowInspectorGUIPanel(){
			...
			GUILayout.BeginArea(inspectorRect, (selectedNode != null? selectedNode.name : "Connection"), (GUIStyle)"editorPanel");
			GUILayout.Space(5);
			GUI.skin = null;
