...
namespace NodeCanvas.Framework{
	...
	abstract public partial class Node {
		...
		virtual public string name{
			get
			{
				...
				if (string.IsNullOrEmpty(_nodeName) ){
					...
					_nodeName = nameAtt != null? nameAtt.name : GetType().FriendlyName();
				}
				return _nodeName;
			}
		...
		public void SubscribeToMessage(params string[] messages){
			var utils = graphAgent.GetComponent<MessageRouter>();
			if (utils == null)
				utils = graphAgent.gameObject.AddComponent<MessageRouter>();
			for (var i = 0; i < messages.Length; i++)
				utils.Listen(this, messages[i]);
		}

		///Unsubscrive the node from all messages send to the agent
