...
namespace NodeCanvas.Framework{
	...
	abstract public class BBParameter {
		...
		abstract public void Bind(Variable data);
		...
		public override string ToString(){
			...
			if (objectValue is string)
				return string.Format("\"{0}\"", objectValue.ToString());
	...
	public class BBParameter<T> : BBParameter{
		...
		public override void Bind(Variable data){
			if (data == null){
				getter = null;
				setter = null;
				if (useBlackboard && Application.isPlaying){
					_value = default(T);
				}
				return;
			}

			if (Application.isPlaying && !typeof(T).RTIsAssignableFrom(data.varType) && !data.varType.RTIsAssignableFrom(typeof(T)) ){
