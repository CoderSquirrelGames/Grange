...
namespace ParadoxNotion.Serialization.FullSerializer.Internal {
        ...
        public void Add(object collection, object item) {
            ...
            if (pair.Key == null || pair.Value == null) {
                ...
                if (collectionType != null) {
                    ...
                    var keyValueInstance = Activator.CreateInstance(keyValuePairType, pair.Key, pair.Value);
    ...
    public class fsIEnumerableConverter : fsConverter {
        private readonly fsIEnumerableSerializationAdapter[] _adaptors = new fsIEnumerableSerializationAdapter[] {
            new fsIDictionaryAdapter(),
            new fsReflectedAdapter()
        };

        private fsIEnumerableSerializationAdapter GetAdapter(Type type) {
            for (var i = 0; i < _adaptors.Length; ++i) {
        ...
        public override fsResult TrySerialize(object instance, out fsData serialized, Type storageType) {
            ...
            var adapter = GetAdapter(storageType);
            var elementType = adapter.GetElementType(storageType);

            serialized = fsData.CreateList(adapter.HintSize(instance));
            var serializedList = serialized.AsList;

            foreach (var item in adapter.Iterate(instance)) {
